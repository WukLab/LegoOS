...
//#include <lego/mii.h>
//#include <lego/phy.h>
//#include <lego/brcmphy.h>
//#include <lego/if_vlan.h>
//#include <lego/ip.h>
//#include <lego/tcp.h>
...
//#include <lego/prefetch.h>
#include <lego/dma-mapping.h>
//#include <lego/firmware.h>
...
static void tg3_adjust_link(struct pci_dev *pdev)
{
...
static int tg3_power_down_prepare(struct tg3 *tp)
{
	...
	if (!(tg3_flag(tp, 5750_PLUS) && tg3_flag(tp, ENABLE_ASF))) {
...
static void tg3_dump_state(struct tg3 *tp)
{
	...
	if (!regs) {
		tg3_err(tp->dev, "Failed allocating register dump buffer\n");
		return;
	}
	...
	for (i = 0; i < TG3_REG_BLK_SIZE / sizeof(u32); i += 4) {
		...
		tg3_err(tp->dev, "0x%08x: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
			   i * 4,
...
static void tg3_process_error(struct tg3 *tp)
{
	...
	if (val & ~HOSTCC_FLOW_ATTN_MBUF_LWM) {
		tg3_err(tp->dev, "FLOW Attention error.  Resetting chip.\n");
		real_error = true;
	}

	if (tr32(MSGINT_STATUS) & ~MSGINT_STATUS_MSI_REQ) {
		tg3_err(tp->dev, "MSI Status error.  Resetting chip.\n");
		real_error = true;
	}

	if (tr32(RDMAC_STATUS) || tr32(WDMAC_STATUS)) {
		tg3_err(tp->dev, "DMA Status error.  Resetting chip.\n");
		real_error = true;
	}
...
static void tg3_tx_timeout(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;

	if (netif_msg_tx_err(tp)) {
		tg3_err(dev, "transmit timed out, resetting\n");
		tg3_dump_state(tp);
	}
...
static u32 tg3_fix_features(struct pci_dev *pdev, u32 features)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_features(struct pci_dev *pdev, u32 features)
{
...
static inline void tg3_set_mtu(struct pci_dev *pdev, struct tg3 *tp,
			       int new_mtu)
{
	dev->mtu = new_mtu;
...
static int tg3_change_mtu(struct pci_dev *pdev, int new_mtu)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int err;
...
static int tg3_poll_fw(struct tg3 *tp)
{
	...
	if (i >= 100000 && !tg3_flag(tp, NO_FWARE_REPORTED)) {
		...
		tg3_info(tp->dev, "No firmware running\n");
	}
...
static int tg3_halt_cpu(struct tg3 *tp, u32 offset)
{
	...
	if (i >= 10000) {
		tg3_err(tp->dev, "%s timed out, %s CPU\n",
...
static int tg3_set_mac_addr(struct pci_dev *pdev, void *p)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_reset_hw(struct tg3 *tp, int reset_phy)
{
	...
	if (i >= 2000) {
		tg3_err(tp->dev, "%s cannot enable BUFMGR\n", __func__);
		return -ENODEV;
	}
...
static int tg3_test_msi(struct tg3 *tp)
{
	...
	tg3_warn(tp->dev, "No interrupt was generated using MSI. Switching "
...
static int tg3_request_firmware(struct tg3 *tp)
{
	...
	if (request_firmware(&tp->fw, tp->fw_needed, &tp->pdev->dev)) {
		tg3_err(tp->dev, "Failed to load firmware \"%s\"\n",
			   tp->fw_needed);
		return -ENOENT;
	}
	...
	if (tp->fw_len < (tp->fw->size - 12)) {
		tg3_err(tp->dev, "bogus length %d in \"%s\"\n",
...
static int tg3_open(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int i, err;
	...
	if (tp->fw_needed) {
		...
		if (tp->pci_chip_rev_id == CHIPREV_ID_5701_A0) {
			...
			tg3_warn(tp->dev, "TSO capability disabled\n");
			tg3_flag_clear(tp, TSO_CAPABLE);
		} else if (!tg3_flag(tp, TSO_CAPABLE)) {
...
static int tg3_close(struct pci_dev *pdev)
{
	int i;
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static struct rtnl_link_stats64 *tg3_get_stats64(struct pci_dev *pdev,
...
static void __tg3_set_rx_mode(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	u32 rx_mode;
...
static void tg3_set_rx_mode(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_get_regs_len(struct pci_dev *pdev)
{
	return TG3_REG_BLK_SIZE;
}

static void tg3_get_regs(struct pci_dev *pdev,
...
static int tg3_get_eeprom_len(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_get_eeprom(struct pci_dev *pdev, struct ethtool_eeprom *eeprom, u8 *data)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int ret;
	u8  *pd;
...
static int tg3_set_eeprom(struct pci_dev *pdev, struct ethtool_eeprom *eeprom, u8 *data)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int ret;
...
static int tg3_get_settings(struct pci_dev *pdev, struct ethtool_cmd *cmd)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_settings(struct pci_dev *pdev, struct ethtool_cmd *cmd)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static void tg3_get_drvinfo(struct pci_dev *pdev, struct ethtool_drvinfo *info)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static void tg3_get_wol(struct pci_dev *pdev, struct ethtool_wolinfo *wol)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_wol(struct pci_dev *pdev, struct ethtool_wolinfo *wol)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static u32 tg3_get_msglevel(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	return tp->msg_enable;
}

static void tg3_set_msglevel(struct pci_dev *pdev, u32 value)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	tp->msg_enable = value;
}
...
#if 0
static int tg3_nway_reset(struct pci_dev *pdev)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int r;
...
static void tg3_get_ringparam(struct pci_dev *pdev, struct ethtool_ringparam *ering)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_ringparam(struct pci_dev *pdev, struct ethtool_ringparam *ering)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static void tg3_get_pauseparam(struct pci_dev *pdev, struct ethtool_pauseparam *epause)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_pauseparam(struct pci_dev *pdev, struct ethtool_pauseparam *epause)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
	int err = 0;
...
static int tg3_get_sset_count(struct pci_dev *pdev, int sset)
{
	switch (sset) {
	case ETH_SS_TEST:
		return TG3_NUM_TEST;
...
static void tg3_get_strings(struct pci_dev *pdev, u32 stringset, u8 *buf)
{
	switch (stringset) {
	case ETH_SS_STATS:
...
static int tg3_set_phys_id(struct pci_dev *pdev,
...
static void tg3_get_ethtool_stats(struct pci_dev *pdev,
...
static int tg3_get_coalesce(struct pci_dev *pdev, struct ethtool_coalesce *ec)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
static int tg3_set_coalesce(struct pci_dev *pdev, struct ethtool_coalesce *ec)
{
	struct tg3 *tp = (struct tg3 *)pdev->priv;
...
XXX need to set static mac
	...
	err = tg3_test_dma(tp);
	if (err) {
		dev_err(pdev, "DMA engine test failed, aborting\n");
	...
	tg3_info(dev, "Tigon3 [partno(%s) rev %04x] (%s) MAC address %pM\n",
	...
	if (tp->phy_flags & TG3_PHYFLG_IS_CONNECTED) {
		...
		tg3_info(dev, "attached PHY is %s (%s Ethernet) "
			    "(WireSpeed[%d], EEE[%d])\n",
	...
	tg3_info(dev, "RXcsums[%d] LinkChgREG[%d] MIirq[%d] ASF[%d] TSOcap[%d]\n",
		    (dev->features & NETIF_F_RXCSUM) != 0,
		    tg3_flag(tp, USE_LINKCHG_REG) != 0,
		    (tp->phy_flags & TG3_PHYFLG_USE_MI_INTERRUPT) != 0,
		    tg3_flag(tp, ENABLE_ASF) != 0,
		    tg3_flag(tp, TSO_CAPABLE) != 0);
	tg3_info(dev, "dma_rwctrl[%08x] dma_mask[%d-bit]\n",
		    tp->dma_rwctrl,
...
{
//	struct pci_dev *pdev = pci_get_drvdata(pdev);

//	if (dev) {
		struct tg3 *tp = pdev->priv;
