...
#define GET_NODE_ID_FROM_POST_RECEIVE_ID(id)                                   \
	(id >> 8) / (NUM_PARALLEL_CONNECTION + 1)
#else
...
#define LID_SEND_RECV_FORMAT                                                   \
	"0000:0000:000000:000000:00000000000000000000000000000000"
#ifdef CONFIG_SOCKET_O_IB
#define MAX_CONNECTION                                                         \
	MAX_NODE *(NUM_PARALLEL_CONNECTION +                                   \
		   1) //Assume that MAX_CONNECTION is smaller than 256
#else
#define MAX_CONNECTION                                                         \
	MAX_NODE *NUM_PARALLEL_CONNECTION //Assume that MAX_CONNECTION is smaller than 256
#endif
#define MAX_PARALLEL_THREAD 64
#define WRAP_UP_NUM_FOR_WRID                                                   \
	256 //since there are 64 bits in wr_id, we are going to use 9-12 bits to do thread id waiting passing
#define WRAP_UP_NUM_FOR_CIRCULAR_ID 256
#define WRAP_UP_NUM_FOR_WAITING_INBOX 256
#define WRAP_UP_NUM_FOR_TYPE                                                   \
	65536 //since there are 64 bits in wr_id, we are going to use 9-12 bits to do thread id waiting passing
...
struct lego_context {
	...
	int (*send_reply_handler)(char *input_addr, uint32_t input_size,
				  char *output_addr, uint32_t *output_size,
				  int sender_id);
	int (*send_reply_opt_handler)(char *input_buf, uint32_t size,
				      void **output_buf, uint32_t *output_size,
				      int sender_id);
	int (*send_reply_rdma_imm_handler)(int sender_id, void *msg,
					   uint32_t size, uint32_t inbox_addr,
					   uint32_t inbox_rkey,
					   uint32_t inbox_semaphore);
	...
	int imm_perport_reg_num
		[IMM_MAX_PORT]; //-1 no registeration, 0 up --> how many
	spinlock_t imm_waitqueue_perport_lock[IMM_MAX_PORT];

#ifdef CONFIG_SOCKET_O_IB
	struct imm_header_from_cq_to_port
		sock_imm_waitqueue_perport[SOCK_MAX_LISTEN_PORTS];
