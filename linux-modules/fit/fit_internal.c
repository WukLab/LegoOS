...
int fit_send_message_sge(struct lego_context *ctx, int connection_id, int type,
			 void *addr, int size, uint64_t inbox_addr,
			 uint64_t inbox_semaphore, int priority);
#if 0
//LOCK related
...
int init_socket_over_ib(struct lego_context *ctx, int port, int rx_depth, int i)
{
	...
	ctx->sock_send_cq[i] = ib_create_cq((struct ib_device *)ctx->context,
					    NULL, NULL, NULL, rx_depth + 1, 0);
	//ctx->send_cq[i] = ctx->send_cq[0];
	struct ib_qp_attr attr;
	struct ib_qp_init_attr init_attr = { .send_cq = ctx->sock_send_cq[i],
	...
	struct ib_qp_attr attr1 = { .qp_state = IB_QPS_INIT,
				    .pkey_index = 0,
				    .port_num = port,
...
struct lego_context *fit_init_ctx(int size, int rx_depth, int port,
				  struct ib_device *ib_dev, int mynodeid)
{
	...
	printk(KERN_CRIT "%s\n", __func__);
	ctx = (struct lego_context *)kzalloc(sizeof(struct lego_context),
					     GFP_KERNEL);
	if (!ctx) {
	...
	ctx->proc = ctx->pd->device->ops.get_dma_mr(
		ctx->pd, IB_ACCESS_LOCAL_WRITE | IB_ACCESS_REMOTE_WRITE |
				 IB_ACCESS_REMOTE_READ);
	//ctx->proc = ctx->pd->device->get_dma_mr(ctx->pd, IB_ACCESS_LOCAL_WRITE | IB_ACCESS_REMOTE_WRITE | IB_ACCESS_REMOTE_READ);
#else
	ctx->proc = ib_get_dma_mr(ctx->pd, IB_ACCESS_LOCAL_WRITE |
						   IB_ACCESS_REMOTE_WRITE |
						   IB_ACCESS_REMOTE_READ);
#endif

	ctx->send_state = (enum s_state *)kmalloc(
		num_connections * sizeof(enum s_state), GFP_KERNEL);
	ctx->recv_state = (enum r_state *)kmalloc(
		num_connections * sizeof(enum r_state), GFP_KERNEL);

	printk(KERN_CRIT "%s proc lkey %d rkey %d\n", __func__, ctx->proc->lkey,
	       ctx->proc->rkey);

	//Customized part
	ctx->num_alive_connection = (atomic_t *)kmalloc(
		ctx->num_node * sizeof(atomic_t), GFP_KERNEL);
	...
	ctx->recv_num =
		(int *)kmalloc(ctx->num_connections * sizeof(int), GFP_KERNEL);
	memset(ctx->recv_num, 0, ctx->num_connections * sizeof(int));

	ctx->atomic_request_num = (atomic_t *)kmalloc(
		ctx->num_node * sizeof(atomic_t), GFP_KERNEL);
	...
	ctx->atomic_buffer = (struct atomic_struct **)kmalloc(
		num_connections * sizeof(struct atomic_struct *), GFP_KERNEL);
	ctx->atomic_buffer_total_length =
		(int *)kmalloc(num_connections * sizeof(int), GFP_KERNEL);
	for (i = 0; i < num_connections; i++)
		ctx->atomic_buffer_total_length[i] = 0;
	ctx->atomic_buffer_cur_length =
		(int *)kmalloc(num_connections * sizeof(int), GFP_KERNEL);
	for (i = 0; i < num_connections; i++)
		ctx->atomic_buffer_cur_length[i] = -1;

	ctx->cq = (struct ib_cq **)kmalloc(
		NUM_POLLING_THREADS * sizeof(struct ib_cq *), GFP_KERNEL);
	for (i = 0; i < NUM_POLLING_THREADS; i++) {
		...
		ctx->cq[i] = ib_create_cq((struct ib_device *)ctx->context,
					  NULL, NULL, NULL, &cq_attr);
#else
		ctx->cq[i] =
			ib_create_cq((struct ib_device *)ctx->context, NULL,
				     NULL, NULL, rx_depth * 4 + 1, 0);
#endif
		if (!ctx->cq[i]) {
	...
	ctx->send_cq = (struct ib_cq **)kmalloc(
		num_connections * sizeof(struct ib_cq *), GFP_KERNEL);
	ctx->connection_count = (atomic_t *)kmalloc(
		num_connections * sizeof(atomic_t), GFP_KERNEL);
	for (i = 0; i < num_connections; i++) {
	...
	printk(KERN_CRIT "%s before create qps numconnections %d\n", __func__,
	       num_connections);
	ctx->qp = (struct ib_qp **)kmalloc(
		num_connections * sizeof(struct ib_qp *), GFP_KERNEL);
	if (!ctx->qp) {
...
#ifdef CONFIG_SOCKET_O_IB
	ctx->sock_send_cq = (struct ib_cq **)kmalloc(
		MAX_NODE * sizeof(struct ib_cq *), GFP_KERNEL);
	ctx->sock_qp = (struct ib_qp **)kmalloc(
		MAX_NODE * sizeof(struct ib_qp *), GFP_KERNEL);
	ctx->sock_recv_cq = ib_create_cq((struct ib_device *)ctx->context, NULL,
					 NULL, NULL, rx_depth + 1, 0);
	...
	for (i = 0; i < num_connections; i++) {
		...
		if (i % (NUM_PARALLEL_CONNECTION + 1) ==
		    NUM_PARALLEL_CONNECTION) {
		...
		ctx->send_cq[i] = ib_create_cq((struct ib_device *)ctx->context,
					       NULL, NULL, NULL, &cq_attr);
#else
		ctx->send_cq[i] =
			ib_create_cq((struct ib_device *)ctx->context, NULL,
	...
	ctx->reply_ready_indicators = (void **)kmalloc(
		sizeof(void *) * IMM_NUM_OF_SEMAPHORE, GFP_KERNEL);
	ctx->reply_ready_indicators_bitmap = kzalloc(
		sizeof(unsigned long) * BITS_TO_LONGS(IMM_NUM_OF_SEMAPHORE),
		GFP_KERNEL);
	...
	ctx->imm_inbox_block_queue = (wait_queue_head_t *)kmalloc(
		(IMM_NUM_OF_SEMAPHORE) * sizeof(wait_queue_head_t), GFP_KERNEL);
	for (i = 0; i < IMM_NUM_OF_SEMAPHORE; i++)
		init_waitqueue_head(&ctx->imm_inbox_block_queue[i]);
#endif
#ifdef SCHEDULE_MODEL
	ctx->thread_waiting_for_reply = (struct task_struct **)kzalloc(
		sizeof(struct task_struct *) * IMM_NUM_OF_SEMAPHORE,
		GFP_KERNEL);
#endif
...
struct lego_context *fit_init_interface(int ib_port, struct ib_device *ib_dev,
					int mynodeid)
{
	int size = 4096;
...
retry:
	ret = ib_query_port((struct ib_device *)ctx->context, ib_port,
			    &ctx->portinfo);
	if (ret < 0) {
...
uintptr_t fit_ib_reg_mr_phys_addr(struct lego_context *ctx, void *addr,
				  size_t length)
{
...
struct fit_ibv_mr *fit_ib_reg_mr(struct lego_context *ctx, void *addr,
				 size_t length, enum ib_access_flags access)
{
	...
	*/
	...
	ret = (struct fit_ibv_mr *)kmalloc(sizeof(struct fit_ibv_mr),
					   GFP_KERNEL);

	ret->addr = (void *)ib_dma_map_single((struct ib_device *)ctx->context,
...
inline uintptr_t fit_ib_reg_mr_addr_phys(struct lego_context *ctx, void *addr,
					 size_t length)
{
	return fit_ib_reg_mr_phys_addr(ctx, addr, length);
}

inline uintptr_t fit_ib_reg_mr_addr(struct lego_context *ctx, void *addr,
				    size_t length)
{
...
{
	return ib_dma_unmap_single((struct ib_device *)ctx->context,
				   (uint64_t)addr, length, DMA_BIDIRECTIONAL);
...
static int fit_post_receives_message(struct lego_context *ctx,
				     int connection_id, int depth)
{
	int i, ret;

	for (i = 0; i < depth; i++) {
		struct ib_recv_wr wr, *bad_wr = NULL;
		wr.wr_id = i + (connection_id
				<< CONNECTION_ID_PUSH_BITS_BASED_ON_RECV_DEPTH);
		wr.next = NULL;
		wr.sg_list = NULL;
		wr.num_sge = 0;
...
#ifdef CONFIG_SOCKET_O_IB
int sock_post_receives_message(struct lego_context *ctx, int connection_id,
			       int depth)
{
	int i, ret;
...
int sock_post_receive_buffer(struct lego_context *ctx, int connection_id,
			     int depth)
{
	int i;
	char *buf;
...
int fit_post_receives_message_with_buffer(struct lego_context *ctx,
					  int connection_id, int depth)
{
	int i;
	char *buf, *header;
	...
	for (i = 0; i < depth; i++) {
		...
		header_addr = fit_ib_reg_mr_addr(ctx, header,
						 sizeof(struct ibapi_header));
		p_r_i_struct = (struct ibapi_post_receive_intermediate_struct *)kmalloc(
			sizeof(struct ibapi_post_receive_intermediate_struct),
			GFP_KERNEL);
		p_r_i_struct->header = (uintptr_t)header;
		p_r_i_struct->msg = (uintptr_t)buf;

		printk(KERN_CRIT
		       "%s pristruct %p header %p buf %p msg %p byf %p\n",
		       __func__, p_r_i_struct, p_r_i_struct->header, buf,
		       p_r_i_struct->msg, addr);
		...
		if (ret) {
			printk(KERN_CRIT
			       "ERROR: %s post recv error %d conn %d i %d\n",
			       __func__, ret, connection_id, i);
		}
		printk(KERN_CRIT
		       "%s header %p header_addr %p buf %p addr %p lkey %d\n",
		       __func__, header, header_addr, buf, addr,
		       ctx->proc->lkey);
	}
...
#ifdef CONFIG_SOCKET_O_IB
int connect_sock_qp(struct lego_context *ctx, int connection_id, int port,
		    enum ib_mtu mtu, int sl, int destlid, int destqpn)
{
	struct ib_qp_attr attr = {
		...
		.min_rnr_timer = 12,
		.ah_attr = { .dlid = destlid, .sl = sl, .port_num = port }
	};
...
int fit_connect_ctx(struct lego_context *ctx, int connection_id, int port,
		    enum ib_mtu mtu, int sl, int destlid, int destqpn)
{
	struct ib_qp_attr attr = {
		.qp_state	= IB_QPS_RTR,
...
int sock_connect_nodes(struct lego_context *ctx, int rem_node_id, int mynodeid)
{
	 ...
	 */
	global_qpn = get_global_qpn(ctx->node_id, rem_node_id,
				    NUM_PARALLEL_CONNECTION);
	printk(KERN_ALERT
	       "%s: mynode %d remnode %d remotelid %d remoteqpn %d\n",
	       __func__, ctx->node_id, rem_node_id, global_lid[rem_node_id],
	       global_qpn);
retry:
	ret = connect_sock_qp(ctx, rem_node_id, ib_port, mtu, sl,
			      global_lid[rem_node_id], global_qpn);
	if (ret) {
...
#ifdef CONFIG_SOCKET_O_IB
		cur_connection =
			(rem_node_id * (ctx->num_parallel_connection + 1)) +
			atomic_read(&ctx->num_alive_connection[rem_node_id]);
#else
		cur_connection =
			(rem_node_id * ctx->num_parallel_connection) +
			atomic_read(&ctx->num_alive_connection[rem_node_id]);
#endif
		global_qpn = get_global_qpn(ctx->node_id, rem_node_id, i);
		printk(KERN_ALERT
		       "%s: cur connection %d mynode %d remnode %d remotelid %d remoteqpn %d\n",
		       __func__, cur_connection, ctx->node_id, rem_node_id,
		       global_lid[rem_node_id], global_qpn);
	retry:
		ret = fit_connect_ctx(ctx, cur_connection, ib_port, mtu, sl,
				      global_lid[rem_node_id], global_qpn);
		if (ret) {
		...
		if (i == 0)
			fit_post_receives_message_with_buffer(
				ctx, cur_connection, 1); //ctx->num_node - 1);

		/* post receive buffers for IMM */
		fit_post_receives_message(ctx, cur_connection,
					  ctx->rx_depth / 2);

		atomic_inc(&ctx->num_alive_connection[rem_node_id]);
		atomic_inc(&ctx->alive_connection);
		if (atomic_read(&ctx->num_alive_connection[rem_node_id]) ==
		    NUM_PARALLEL_CONNECTION) {
...
#ifdef CONFIG_SOCKET_O_IB
inline int fit_find_sock_qp_id_by_qpnum(struct lego_context *ctx,
					uint32_t qp_num)
{
	int i;
...
int fit_internal_poll_sendcq(struct ib_cq *tar_cq, int connection_id,
			     int *check)
{
	...
	do {
		...
		if (ne < 0) {
			printk(KERN_ALERT
			       "poll send_cq failed at connection %d\n",
			       connection_id);
			return 1;
		}

		if (unlikely(sched_clock() - start_ns >
			     FIT_POLL_SENDCQ_TIMEOUT_NS)) {
			pr_info_once(
				"\n"
				"*****\n"
				...
				connection_id,
				connection_id / NUM_PARALLEL_CONNECTION);
			WARN_ON_ONCE(1);
		}
	} while (ne < 1);
	for (i = 0; i < ne; i++) {
		if (wc[i].status != IB_WC_SUCCESS) {
			printk(KERN_ALERT
			       "send request failed at connection %d as %d\n",
...
#ifdef NEW_IB_API
int write_with_imm(struct lego_context *ctx, int connection_id,
		   ...
		   struct imm_message_metadata *header, int userspace_flag)
{
	...
	if (s_mode == FIT_SEND_MESSAGE_HEADER_AND_IMM) {
		wr->wr_id =
			(uint64_t)ctx->reply_ready_indicators
				[header->inbox_semaphore]; //get the real wait_send_reply_id address from inbox information
		wr->send_flags = IB_SEND_SIGNALED;
		wr->num_sge = 2;
		wr->opcode = IB_WR_RDMA_WRITE_WITH_IMM;

		temp_header_addr = fit_ib_reg_mr_addr(
			ctx, header, sizeof(struct imm_message_metadata));
		wr->ex.imm_data = imm;
	...
	if (!ret) {
		fit_internal_poll_sendcq(ctx->send_cq[connection_id],
...
inline int fit_get_connection_by_atomic_number(struct lego_context *ctx,
					       int target_node, int priority)
{
#ifdef CONFIG_SOCKET_O_IB
	return atomic_inc_return(&ctx->atomic_request_num[target_node]) %
		       (atomic_read(&ctx->num_alive_connection[target_node])) +
			...
			uintptr_t *reply_descriptor, int userspace_flag)
{
	...
	while (1) {
		...
		if (!list_empty(&(ctx->imm_waitqueue_perport[port].list))) {
			...
			new_request = list_entry(
				ctx->imm_waitqueue_perport[port].list.next,
	...
	tmp = (struct imm_message_metadata
		       *)(ctx->local_rdma_recv_rings[node_id] + offset);
	get_size = tmp->size;
	...
	//do data memcpy
	memcpy(ret_addr, ((void *)tmp) + sizeof(struct imm_message_metadata),
	       get_size);
	...
	descriptor = (struct imm_message_metadata *)kmalloc(
		sizeof(struct imm_message_metadata),
		GFP_KERNEL); //kmem_cache_alloc(imm_message_metadata_cache, GFP_KERNEL);
	WARN_ON(!descriptor);
	/*
	while(!descriptor)
	{
	...
	*reply_descriptor = (uintptr_t)descriptor;
	...
	if ((offset >= last_ack && offset - last_ack >= IMM_ACK_FREQ) ||
	    (offset < last_ack &&
	     offset + IMM_PORT_CACHE_SIZE - last_ack >= IMM_ACK_FREQ)) {
		ack_flag = 1;
	...
	if (ack_flag) {
		struct send_and_reply_format *pass =
			(struct send_and_reply_format *)kmalloc(
				sizeof(struct send_and_reply_format),
				GFP_KERNEL);
		      ...
		      uintptr_t descriptor, int userspace_flag)
{
	struct imm_message_metadata *tmp =
		(struct imm_message_metadata *)descriptor;
	int re_connection_id = fit_get_connection_by_atomic_number(
		ctx, tmp->source_node_id, LOW_PRIORITY);
	unsigned long phys_addr;
	void *real_addr;
	struct ib_device *ibd = (struct ib_device *)ctx->context;

	write_with_imm(ctx, re_connection_id, tmp->inbox_rkey, tmp->inbox_addr,
		       addr, size, 0,
		       tmp->inbox_semaphore | IMM_SEND_REPLY_RECV,
...
get_next_request:
	...
	while (1) {
		...
		list_for_each_entry_safe (
			...
			offset = new_request->offset;
			printk(KERN_CRIT
			       "got new req offset %d sourcenode %d size %d\n",
	...
	if ((sock_type & O_NONBLOCK) > 0 && get_size == 0) {
		printk(KERN_CRIT
		       "nonblock socket return when running out of received buffer %d\n",
		       total_received_size);
		return total_received_size;
	}
	...
	*/
	if (if_userspace) {
		...
		cp_ret = copy_to_user(ret_addr,
				      ctx->local_sock_rdma_recv_rings[node_id] +
					      offset,
				      get_size);
		WARN_ON(cp_ret);
	} else
		memcpy(ret_addr,
		       ctx->local_sock_rdma_recv_rings[node_id] + offset,
		       get_size);
	...
	if (total_received_size < receive_size) {
		printk(KERN_CRIT
		       "go to next request received size %d total %d\n",
...
int sock_send_message_with_rdma_imm(struct lego_context *ctx, int target_node,
				    uint32_t input_mr_rkey,
				    uintptr_t input_mr_addr, void *addr,
				    int size, int offset, uint32_t imm_data,
				    void *header, int header_size,
				    enum mode s_mode, int if_use_phys_addr_reg)
{
	struct ib_send_wr wr, *bad_wr = NULL;
	struct ib_sge sge[2];
	int ret, i, ne;
	...
	printk(KERN_CRIT
	       "%s target_node %d rkey %d mraddr %lx addr %p size %d offset %d imm-0x%x mode %d\n",
	       __func__, target_node, input_mr_rkey, input_mr_addr, addr, size,
	       offset, imm_data, s_mode);
	memset(&wr, 0, sizeof(struct ib_send_wr));
	memset(&sge, 0, sizeof(struct ib_sge));
	...
	printk(KERN_CRIT
	       "wr: remotr_addr: %p, rkey: %#lx header %p header size %d\n",
	...
	if (!ret) {
		do {
			...
			if (ne < 0) {
				printk(KERN_ALERT
				       "poll send_cq failed at send-qp\n");
...
static int fit_poll_cq(struct lego_context *ctx, struct ib_cq *target_cq)
{
	...
	while (1) {
		...
		for (i = 0; i < ne; i++) {
			...
			if ((int)wc[i].opcode == IB_WC_RECV) {
			...
			} else if ((int)wc[i].opcode ==
				   IB_WC_RECV_RDMA_WITH_IMM) {
				node_id = GET_NODE_ID_FROM_POST_RECEIVE_ID(
					wc[i].wr_id);

				if (wc[i].wc_flags && IB_WC_WITH_IMM) {
					if (wc[i].ex.imm_data &
					    IMM_SEND_REPLY_SEND) {
						offset = wc[i].ex.imm_data &
							 IMM_GET_OFFSET;
						port = IMM_GET_PORT_NUMBER(
							wc[i].ex.imm_data);

						tmp = kmalloc(
							sizeof(struct imm_header_from_cq_to_port),
							GFP_KERNEL);
						if (!tmp) {
							WARN_ON(1);
							return -ENOMEM;
						...
						spin_lock(
							&ctx->imm_waitqueue_perport_lock
								 [port]);
						list_add_tail(
							&(tmp->list),
							&ctx->imm_waitqueue_perport
								 [port]
									 .list);
						spin_unlock(
							&ctx->imm_waitqueue_perport_lock
								 [port]);
					} else if (wc[i].ex.imm_data &
							   IMM_ACK ||
						   wc[i].byte_len == 0) {
						/* Internal ACK */
						offset = wc[i].ex.imm_data &
						...
						semaphore = wc[i].ex.imm_data &
							    IMM_GET_SEMAPHORE;
						if (semaphore < 0 ||
						    semaphore >=
							    IMM_NUM_OF_SEMAPHORE) {
							pr_err("Wrong index: %d\n",
							       semaphore);
							WARN_ON_ONCE(1);
							continue;
						}
						memcpy((void *)ctx->reply_ready_indicators
							       [semaphore],
						       &length, sizeof(int));

						ctx->reply_ready_indicators
							[semaphore] = NULL;
						clear_bit(
							semaphore,
							ctx->reply_ready_indicators_bitmap);
					} else {
				...
				if (GET_POST_RECEIVE_DEPTH_FROM_POST_RECEIVE_ID(
					    wc[i].wr_id) %
					    (ctx->rx_depth / 4) ==
				    ((ctx->rx_depth / 4) - 1)) {
					connection_id = fit_find_qp_id_by_qpnum(
						ctx, wc[i].qp->qp_num);
					if (connection_id == -1) {
				...
				connection_id = fit_find_qp_id_by_qpnum(
					ctx, wc[i].qp->qp_num);
				printk(KERN_ALERT
				       "%s: connection %d Recv weird event as %d\n",
...
int waiting_queue_handler(void *in)
{
	...
	while (1) {
		...
		spin_lock(&wq_lock);
		new_request = list_entry(request_list.list.next,
		...
		case MSG_DO_RC_POST_RECEIVE:
			...
			fit_post_receives_message(ctx, new_request->src_id,
						  new_request->length);
			break;
		case MSG_DO_ACK_INTERNAL: {
			...
			write_with_imm(
				ctx,
				target_node * (NUM_PARALLEL_CONNECTION + 1), 0,
				0, 0, 0, 0, offset, FIT_SEND_ACK_IMM_ONLY, NULL,
				FIT_KERNELSPACE_FLAG);
#else
			write_with_imm(ctx,
				       target_node * NUM_PARALLEL_CONNECTION, 0,
				       0, 0, 0, 0, offset,
				       FIT_SEND_ACK_IMM_ONLY, NULL,
				       FIT_KERNELSPACE_FLAG);
#endif
			break;
		}
		case MSG_DO_ACK_REMOTE: {
			...
			ctx->remote_last_ack_index[new_request->src_id] =
				last_ack;
			break;
		}
		default:
...
void fit_setup_ibapi_header(uint32_t src_id, uint64_t inbox_addr,
			    uint64_t inbox_semaphore, uint32_t length,
...
int fit_send_cq_poller(struct lego_context *ctx)
{
	...
	while (1) {
		do {
			...
			if (ne < 0) {
				printk(KERN_ALERT
				       "%s: poll send_cq polling failed at connection\n",
				       __func__);
			}
			if (ne == 0) {
				schedule();
		...
		for (i = 0; i < ne; i++) {
			if (wc[i].status != IB_WC_SUCCESS) {
				printk(KERN_ALERT
				       "%s: send request failed at id %llu as %d\n",
...
#ifdef NEW_IB_API
int fit_send_request(struct lego_context *ctx, int connection_id,
		     enum mode s_mode, struct fit_ibv_mr *input_mr, void *addr,
		     int size, int offset, int userspace_flag)
{
	struct ib_rdma_wr rdma_wr;
	struct ib_send_wr *wr, *bad_wr = NULL;
	struct ib_sge sge;
...
inline int fit_get_inbox_by_addr(struct lego_context *ctx, void *addr)
{
	...
	tar = find_first_zero_bit(ctx->reply_ready_indicators_bitmap,
				  IMM_NUM_OF_SEMAPHORE);
					    ...
					    int if_use_ret_phys_addr)
{
	...
	if (size + sizeof(struct imm_message_metadata) > IMM_MAX_SIZE) {
		printk(KERN_CRIT
		       "%s: message size %d + header is larger than max size %d\n",
	...
	if (ctx->remote_rdma_ring_mrs_offset[target_node] + real_size >=
	    RDMA_RING_SIZE) //If hits the end of ring, write start from 0 directly
		ctx->remote_rdma_ring_mrs_offset[target_node] =
			real_size; //Record the last point
	else
		ctx->remote_rdma_ring_mrs_offset[target_node] += real_size;
	tar_offset_start = ctx->remote_rdma_ring_mrs_offset[target_node] -
			   real_size; //Trace back to the real starting point
	...
	while (1) {
		...
		if (tar_offset_start < last_ack &&
		    tar_offset_start + real_size > last_ack)
			schedule();
		else
			break;
	}
	...
	connection_id = fit_get_connection_by_atomic_number(ctx, target_node,
							    LOW_PRIORITY);
	...
	if (if_use_ret_phys_addr == 1)
		output_header.inbox_addr = fit_ib_reg_mr_addr_phys(
			ctx, ret_addr,
			max_ret_size); //This part need to be handled careful in the future
	else
		output_header.inbox_addr = fit_ib_reg_mr_addr(
			ctx, ret_addr,
			max_ret_size); //This part need to be handled careful in the future
...
#endif
	write_with_imm(ctx, connection_id, remote_rkey, (uintptr_t)remote_addr,
		       addr, size, tar_offset_start, imm_data,
		       FIT_SEND_MESSAGE_HEADER_AND_IMM, &output_header,
		       FIT_KERNELSPACE_FLAG);
	...
	if (wait_send_reply_id == SEND_REPLY_WAIT) {
		while (wait_send_reply_id == SEND_REPLY_WAIT) {
			if (wait_event_interruptible_timeout(
				    ctx->imm_inbox_block_queue[inbox_id],
	...
	if (wait_send_reply_id < 0) {
		printk(KERN_CRIT
		       "%s: [significant error] send-reply-imm fail with connection-%d inbox-%d status-%d\n",
...
#ifdef NEW_IB_API
int fit_send_request_without_polling(struct lego_context *ctx,
				     int connection_id, enum mode s_mode,
...
int fit_send_request_polling_only(struct lego_context *ctx, int connection_id,
				  int polling_num, struct ib_wc *wc)
{
	int ne, i;
...
int fit_send_test(struct lego_context *ctx, int connection_id, int type,
		  void *addr, int size, uint64_t inbox_addr,
		  uint64_t inbox_semaphore, int priority)
{
	struct ib_send_wr wr, *bad_wr = NULL;
	...
	printk(KERN_CRIT "%s conn %d addr %p size %d sendcq %p\n", __func__,
	       connection_id, addr, size, ctx->send_cq[connection_id]);
	spin_lock(&connection_lock[connection_id]);
	...
	if (ret == 0) {
		...
		for (i = 0; i < ne; i++) {
			if (wc[i].status != IB_WC_SUCCESS) {
				printk(KERN_ALERT
				       "send failed at connection %d as %d\n",
				       connection_id, wc[i].status);
				spin_unlock(&connection_lock[connection_id]);
				return 2;
			}
			 ...
			 uint64_t inbox_semaphore, int priority)
{
	...
	fit_setup_ibapi_header(ctx->node_id, inbox_addr, inbox_semaphore, size,
			       priority, type, &output_header);
	output_header_addr = (void *)fit_ib_reg_mr_addr(
		ctx, &output_header, sizeof(struct ibapi_header));
	...
	printk(KERN_CRIT "%s headeraddr %p %p bufaddr %p %#Lx lkey %d\n",
	       __func__, &output_header, output_header_addr, addr, sge[1].addr,
	       ctx->proc->lkey);
	if (ret == 0) {
		do {
			ne = ib_poll_cq(ctx->send_cq[connection_id], 1, wc);
...
int send_rdma_ring_mr_to_other_nodes(struct lego_context *ctx)
{
	...
	for (i = 0; i < ctx->num_node; i++) {
		if (ctx->node_id == i)
			continue;
		memcpy(msg, &ctx->local_rdma_ring_mrs[i],
		       sizeof(struct fit_ibv_mr));
#ifdef CONFIG_SOCKET_O_IB
		connection_id = (NUM_PARALLEL_CONNECTION + 1) * i;
		memcpy(msg + sizeof(struct fit_ibv_mr),
		       &ctx->local_sock_rdma_ring_mrs[i],
		       sizeof(struct fit_ibv_mr));
#else
		...
		ret = fit_send_message_sge(ctx, connection_id,
					   MSG_SEND_RDMA_RING_MR, msg,
					   sizeof(struct fit_ibv_mr), 0, 0,
					   LOW_PRIORITY);
	}
	kfree(msg);
					...
					int mynodeid)
{
	...
	if (temp_ctx_number >= MAX_FIT_NUM) {
		printk(KERN_CRIT
		       "%s Error: already meet the upper bound of connected FIT %d\n",
...
#ifdef SEPARATE_SEND_POLL_THREAD
	thread = kthread_create((void *)fit_send_cq_poller, ctx,
				"separate_poll_send");
	if (IS_ERR(thread)) {
	 ...
	 */
	ctx->local_rdma_recv_rings =
		kmalloc(MAX_NODE * sizeof(void *), GFP_KERNEL);
	ctx->local_rdma_ring_mrs = (struct fit_ibv_mr *)kmalloc(
		MAX_NODE * sizeof(struct fit_ibv_mr), GFP_KERNEL);
	for (i = 0; i < MAX_NODE; i++) {
		ctx->local_rdma_recv_rings[i] =
			fit_alloc_memory_for_mr(IMM_PORT_CACHE_SIZE);
		ret_mr = fit_ib_reg_mr(
			ctx, ctx->local_rdma_recv_rings[i], IMM_RING_SIZE,
			IB_ACCESS_LOCAL_WRITE | IB_ACCESS_REMOTE_WRITE |
				IB_ACCESS_REMOTE_READ);
		memcpy(&ctx->local_rdma_ring_mrs[i], ret_mr,
		       sizeof(struct fit_ibv_mr));
		printk(KERN_CRIT
		       "allocated local recv mr for node %d addr %p %p lkey %d rkey %d",
		       i, ctx->local_rdma_recv_rings[i], ret_mr->addr,
		       ret_mr->lkey, ret_mr->rkey);
	}
	/* array to store rdma ring mr for all remote nodes */
	ctx->remote_rdma_ring_mrs = (struct fit_ibv_mr *)kmalloc(
		MAX_NODE * sizeof(struct fit_ibv_mr), GFP_KERNEL);
	ctx->remote_rdma_ring_mrs_offset =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->remote_last_ack_index =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->local_last_ack_index =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->local_last_ack_index_lock = (spinlock_t *)kmalloc(
		MAX_NODE * sizeof(spinlock_t), GFP_KERNEL);
	ctx->remote_imm_offset_lock = (spinlock_t *)kmalloc(
		MAX_NODE * sizeof(spinlock_t), GFP_KERNEL);
	for (i = 0; i < MAX_NODE; i++) {
	 ...
	 */
	ctx->local_sock_rdma_recv_rings =
		kmalloc(MAX_NODE * sizeof(void *), GFP_KERNEL);
	ctx->local_sock_rdma_ring_mrs = (struct fit_ibv_mr *)kmalloc(
		MAX_NODE * sizeof(struct fit_ibv_mr), GFP_KERNEL);
	for (i = 0; i < MAX_NODE; i++) {
		ctx->local_sock_rdma_recv_rings[i] =
			fit_alloc_memory_for_mr(SOCK_PERNODE_RECV_MR_SIZE);
		ret_mr = fit_ib_reg_mr(ctx, ctx->local_sock_rdma_recv_rings[i],
				       SOCK_PERNODE_RECV_MR_SIZE,
				       IB_ACCESS_LOCAL_WRITE |
					       IB_ACCESS_REMOTE_WRITE |
					       IB_ACCESS_REMOTE_READ);
		memcpy(&ctx->local_sock_rdma_ring_mrs[i], ret_mr,
		       sizeof(struct fit_ibv_mr));
	...
	ctx->remote_sock_rdma_ring_mrs = (struct fit_ibv_mr *)kmalloc(
		MAX_NODE * sizeof(struct fit_ibv_mr), GFP_KERNEL);
	ctx->remote_sock_rdma_ring_mrs_offset =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->remote_sock_last_ack_index =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->local_sock_last_ack_index =
		(int *)kzalloc(MAX_NODE * sizeof(int), GFP_KERNEL);
	ctx->local_sock_last_ack_index_lock = (spinlock_t *)kmalloc(
		MAX_NODE * sizeof(spinlock_t), GFP_KERNEL);
	ctx->remote_sock_imm_offset_lock = (spinlock_t *)kmalloc(
		MAX_NODE * sizeof(spinlock_t), GFP_KERNEL);
	for (i = 0; i < MAX_NODE; i++) {
	...
	printk(KERN_CRIT
	       "[%s] node: %d allocated local rdma buffers, about to connect qps\n",
	       __func__, mynodeid);
	ctx->node_id = mynodeid;
	...
	printk(KERN_ALERT
	       "%s: return before establish connection with NODE_ID: %d\n",
...
{
	Connected_Ctx = (struct lego_context **)kmalloc(
		sizeof(struct lego_context *) * MAX_FIT_NUM, GFP_KERNEL);
